...
function buildgNMIc() {
  ...
  CGO_ENABLED=0 sh ../scripts/go.sh build -o gnmic-rc1 -ldflags="-s -w -X 'github.com/openconfig/gnmic/pkg/version.Commit=$(git rev-parse --short HEAD)' -X 'github.com/openconfig/gnmic/pkg/version.Date=$(date)'" ../
}
