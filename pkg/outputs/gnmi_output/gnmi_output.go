...
func (g *gNMIOutput) Write(ctx context.Context, rsp proto.Message, meta outputs.Meta) {
	var err error
	rsp, err = outputs.AddSubscriptionTarget(rsp, meta, outputs.AddTargetIfNotPresent, g.targetTpl)
	if err != nil {
	...
	// only updates are cached, sync-responses are generated by the server
	// for each of its own subscribers.
	upd := subRsp.GetUpdate()
	if upd == nil {
		return
	}
	target := upd.GetPrefix().GetTarget()
	if target == "" {
