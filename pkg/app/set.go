...
func (a *App) InitSetFlags(cmd *cobra.Command) {
	...
	cmd.Flags().BoolVarP(&a.Config.LocalFlags.SetCommitConfirm, "confirm", "", false, "confirm the commit ID")
	cmd.Flags().BoolVarP(&a.Config.LocalFlags.SetCommitRequest, "request", "", false, "start a commit confirmed transaction")
	cmd.Flags().BoolVarP(&a.Config.LocalFlags.SetCommitCancel, "cancel", "", false, "cancel the commit")
